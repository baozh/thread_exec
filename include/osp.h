...
//OSP版本信息
#define OSPVERSION				"compress OSP.KTM.2800V1R2.1.2.1.20120222"
...
//默认TCP断链检测时间间隔(秒)
#define DEFAULT_TCP_HEARTBEAT_TIME		5
//默认TCP断链检测次数(多次检测均失败认为已经断链)
#define DEFAULT_TCP_HEARTBEAT_NUM		6
//默认OSP系统状态打印时间间隔(秒)
...
//支持消息压缩
OSPEVENT(OSP_COMPRESS_SUPPORT,	0x222); //by wubin 2012-02-22
//压缩消息号
OSPEVENT(OSP_COMPRESS_MSG,	0x223); //by wubin 2012-02-22
// Trc跟踪标志
...
2002/05/23	1.0
=============================================================================*/
API BOOL32 OspOpenLogFile(const char *  szName, u32 maxsize_KB, u32 maxfiles);
// szPath为空则默认为"./"
API BOOL32 OspOpenLogFileEx(const char* szFileName, const char* szPath, u32 maxsize_KB, u32 maxfiles);
...
API BOOL32 OspInit(BOOL32 TelNetEnable=FALSE, u16 TelNetPort=0, char* pchModuleName = NULL , u32 dwMaxNodeNum = 512, u32 dwMaxDispatchMsg = 1024);
...
s16 osplb_create(osplb_handle * handle, lb_create_param * ptParam);
...
API int OspSystem(const char * cmd);	// 嵌入式上不要使用，getpid()问题
...
s32 OspListFile(s8 * pchPath, TListFileInfo *ptFileInfo);
